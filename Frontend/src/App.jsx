...
function App() {
  ...
  return (
    <div className="min-h-screen overflow-hidden bg-black text-slate-100 selection:bg-emerald-300 selection:text-black">
      <Navbar />
      <main>
        <section id="home" className="relative min-h-screen scroll-mt-24 px-4 pt-32 sm:px-6 lg:px-8">
          <BackgroundTexture />
          <div className="relative z-10 mx-auto grid max-w-7xl items-center gap-14 lg:grid-cols-[1.05fr_0.95fr]">
            <motion.div initial={{ opacity: 0, y: 26 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.75 }}>
              ...
              <p className="mt-4 text-2xl font-medium text-emerald-300 sm:text-3xl">{portfolio.role}</p>
              <p className="mt-6 max-w-2xl text-lg leading-8 text-slate-300">{portfolio.tagline}</p>
              <div className="mt-9 flex flex-col gap-3 sm:flex-row">
                <a href="#projects" className="inline-flex items-center justify-center gap-2 rounded-lg bg-emerald-300 px-5 py-3 font-semibold text-black transition hover:bg-emerald-200 focus:outline-none focus:ring-2 focus:ring-emerald-200 focus:ring-offset-2 focus:ring-offset-black">
                  View Projects
              ...
              <div className="mt-8 flex items-center gap-3">
                <SocialLink href={portfolio.socials.github} label="GitHub" icon={GitBranch} />
                <SocialLink href={portfolio.socials.linkedin} label="LinkedIn" icon={Network} />
              </div>
            </motion.div>
            <HeroVisual />
          </div>
        ...
        <MotionSection id="about" className="px-4 sm:px-6 lg:px-8">
          <div className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-[0.8fr_1.2fr]">
            <div>
              <p className="text-sm font-semibold uppercase tracking-[0.24em] text-emerald-300">About Me</p>
              <h2 className="mt-3 text-3xl font-semibold text-white sm:text-4xl">Focused on reliable software that is easy to grow.</h2>
            </div>
            <div className="rounded-lg border border-white/10 bg-white/[0.045] p-6 backdrop-blur-md sm:p-8">
              <p className="text-lg leading-8 text-slate-300">{portfolio.about}</p>
              <div className="mt-8 grid gap-4 sm:grid-cols-3">
                {portfolio.stats.map((stat) => (
                  <div key={stat.label} className="rounded-lg border border-white/10 bg-black/30 p-4">
        ...
        <MotionSection id="skills" className="px-4 sm:px-6 lg:px-8">
          <SectionHeader eyebrow="Skills" title="Practical tools for building production software." description="A compact stack centered on backend development, API design, database work, and polished frontend delivery." />
          <div className="mx-auto grid max-w-7xl gap-4 sm:grid-cols-2 lg:grid-cols-5">
            {portfolio.skills?.map((group, index) => (
              <motion.article key={group.category} custom={index} variants={cardMotion} initial="hidden" whileInView="show" viewport={{ once: true }} className="rounded-lg border border-white/10 bg-white/[0.045] p-5 backdrop-blur-md transition hover:-translate-y-1 hover:border-emerald-300/35 hover:bg-white/[0.07]">
                ...
                <div className="mt-5 flex flex-wrap gap-2">
                  {group.items.map((skill) => (
                    <span key={skill} className="rounded-full border border-white/10 bg-black/35 px-3 py-1.5 text-sm text-slate-200">
                      {skill}
                    </span>
                  ))}
                </div>
        ...
        <MotionSection id="projects" className="px-4 sm:px-6 lg:px-8">
          <SectionHeader eyebrow="Projects" title="Project work with product intent." description="Focused builds that connect frontend experience, backend structure, and maintainable implementation." />
          <div className="mx-auto  grid max-w-7xl gap-10">
            {portfolio.projects.map((project) => (
              <article key={project.name} className="overflow-hidden rounded-lg border border-white/10 bg-white/[0.045] backdrop-blur-md">
                <div className="grid gap-0 lg:grid-cols-[0.8fr_1.2fr]">
                  <div className="relative min-h-72 bg-[linear-gradient(135deg,rgba(16,185,129,0.18),rgba(8,47,73,0.22),rgba(245,158,11,0.12))] p-6">
                    ...
                    <div className="relative flex h-full flex-col justify-between">
                      <div>
                        <p className="text-sm font-medium text-emerald-200">{project.type}</p>
                        <h3 className="mt-3 text-4xl font-semibold text-white">{project.name}</h3>
                      </div>
                      <div className="rounded-lg border border-white/10 bg-black/40 p-4 font-mono text-sm text-slate-300 backdrop-blur-md">
                        <p><span className="text-emerald-300">status</span>: scalable platform build</p>
                        <p><span className="text-cyan-300">stack</span>: MERN Stack + REST APIs</p>
                      </div>
                    </div>
                  </div>
                  <div className="p-6 sm:p-8 ">
                    ...
                    <div className="mt-6 flex flex-wrap gap-2">
                      {project.technologies.map((tech) => (
                        <span key={tech} className="rounded-full bg-white/10 px-3 py-1.5 text-xs font-medium text-slate-200">{tech}</span>
                      ))}
                    </div>
                    <div className="mt-7 flex flex-col gap-3 sm:flex-row">
                      <a href={project.githubUrl} target="_blank" rel="noreferrer" className="inline-flex items-center justify-center gap-2 rounded-lg border border-white/15 px-4 py-2.5 font-medium text-white transition hover:border-emerald-300/45 hover:bg-white/10">
                        <GitBranch size={18} aria-hidden="true" />
                        GitHub
                      </a>
                      <a href={project.liveUrl} className="inline-flex items-center justify-center gap-2 rounded-lg bg-white px-4 py-2.5 font-medium text-black transition hover:bg-emerald-100"
                      target="_blank"
        ...
        <MotionSection id="education" className="px-4 sm:px-6 lg:px-8">
          <SectionHeader eyebrow="Education" title="Academic foundation and continuous learning." />
          <Timeline items={portfolio.education} />
        </MotionSection>

        <MotionSection id="achievements" className="px-4 sm:px-6 lg:px-8">
          <SectionHeader eyebrow="Coding & Achievements" title="Progress markers beyond project code." />
          <div className="mx-auto grid max-w-7xl gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {portfolio.achievements.map((item, index) => (
              <motion.article key={item.title} custom={index} variants={cardMotion} initial="hidden" whileInView="show" viewport={{ once: true }} className="rounded-lg border border-white/10 bg-white/[0.045] p-6 backdrop-blur-md transition hover:-translate-y-1 hover:border-amber-200/40">
                <Award size={24} className="text-amber-200" aria-hidden="true" />
...
/>
            ...
            <form className="rounded-lg border border-white/10 bg-white/[0.045] p-6 backdrop-blur-md" onSubmit={handleContactSubmit}>
              ...
              <label className="mt-4 block">
                ...
                <textarea name="message" rows="5" className="mt-2 w-full rounded-lg border border-white/10 bg-black/35 px-4 py-3 text-white outline-none transition placeholder:text-slate-500 focus:border-emerald-300/60" placeholder="Tell me about the role, project, or idea." required />
              </label>
              ...
              <button type="submit" disabled={isSubmitting} className="mt-5 inline-flex w-full items-center justify-center gap-2 rounded-lg bg-emerald-300 px-5 py-3 font-semibold text-black transition hover:bg-emerald-200 focus:outline-none focus:ring-2 focus:ring-emerald-200 focus:ring-offset-2 focus:ring-offset-black disabled:cursor-not-allowed disabled:opacity-60 sm:w-auto">
...
function SocialLink({ href, label, icon: Icon }) {
  return (
    <a href={href} target="_blank" rel="noreferrer" aria-label={`Open ${label}`} className="inline-flex items-center gap-2 rounded-lg border border-white/10 bg-white/5 px-4 py-2.5 text-sm font-medium text-slate-200 transition hover:border-emerald-300/40 hover:bg-white/10 hover:text-white">
...
function ContactItem({ icon: Icon, label, value, href }) {
  return (
    <a href={href} target={href.startsWith('http') ? '_blank' : undefined} rel={href.startsWith('http') ? 'noreferrer' : undefined} className="group flex items-center gap-4 border-b border-white/10 py-5 first:pt-0 last:border-b-0 last:pb-0">
...
function FormField({ label, name, type = 'text', autoComplete, className = '' }) {
  return (
    <label className={`block ${className}`}>
      <span className="text-sm font-medium text-slate-200">{label}</span>
      <input name={name} type={type} autoComplete={autoComplete} className="mt-2 w-full rounded-lg border border-white/10 bg-black/35 px-4 py-3 text-white outline-none transition placeholder:text-slate-500 focus:border-emerald-300/60" required />
    </label>
  )
}
